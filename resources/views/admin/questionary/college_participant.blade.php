            ...
            <div class="col-sm-6">
                <form id="form-participant" action="{{route('admin.change-data')}}" method="POST" enctype="multipart/form-data" autocomplete="off">
                    ...
                    @if($participant->role_id === 1)
                        ...
                        <div class="form-group row project_direction">
                            ...
                            <div class="col-sm-6">
                                <select class="form-control form-control-sm" id="project_direction" name="project_direction">
                                    @foreach(__('colleges.topic') as $key => $topics)
                        ...
                        <div class="form-group row">
                            <label class="col-sm-6 col-form-label" for="college">{{__('questionary.college.label')}}</label>
                            <div class="col-sm-6">
                                <select class="form-control form-control-sm" id="college" name="college">
                                    @foreach(__('colleges.colleges') as $key => $colleges)
                                        <optgroup label="{{$key}}">
                                            @foreach($colleges as $college)
                                                <option {{ $participant->college == $key . ' ' . $college ? "selected" : "" }} value="{{$key . ' ' . $college}}">{{$college}}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                    <option {{ $participant->college == null ? "selected" : "" }} value="Другое">{{__('questionary.other')}}</option>
                                </select>
                                <div class="other_college mt-2">
                                    <input value="{{$participant->other_college}}" type="text" class="form-control form-control-sm" id="other_college" name="name_college">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group row">
                        <label class="col-sm-6 col-form-label">Факультет/Отделение</label>
                        <div class="col-sm-6">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-6 col-form-label">Курс обучения</label>
                            <div class="col-sm-6">
                            @for ($i = 1; $i < 5; $i++)
                                <div class="form-check">
                                    <input {{ $participant->curs == $i ? "checked" : "" }} class="form-check-input" type="radio" name="kurs" id="{{$i.'_'}}" value="{{$i}}">
                    ...
                    @if($participant->role_id === 1)
                        <div class="form-group row">
                            <label class="col-sm-6 col-form-label" for="fullname_leader">Руководитель</label>
                            <div class="col-sm-6">
                                <input type="text" class="form-control form-control-sm" id="fullname_leader_college" name="fullname_leader_college" value="{{ $participant->leader_fullname }}">
                            </div>
                        </div>
                    @endif
