            ...
            <form id="form-update" action="{{route('updateQuestionary',['id'=>$participant->participant_id])}}" method="POST" autocomplete="off">
                ...
                <div class="form-content">
                    ...
                    @if($participant->role_id===1)
                        ...
                        <p class="font-weight-bold">ВУЗ: <span class="font-weight-light">{{$participant->university}}</span></p>
                    @endif
                    <p class="font-weight-bold">Программа и курс обучения: <span class="font-weight-light">{{$participant->program}}</span></p>
                    ...
                    @if($participant->role_id===1)
                        <div class="form-group">
                            <label for="fullname_leader">Ф.И.О научного руководителя</label>
                            <input type="text" class="form-control" id="fullname_leader" name="fullname_leader" value="{{ $participant->leader_fullname }}">
                            <small class="form-text text-muted font-italic">(Доступен для редактирования)</small>
                        </div>

                        <div class="form-group">
                            <label for="fullname_mentor">Ф.И.О ментора </label>
                            <input type="text" class="form-control" id="fullname_mentor" name="fullname_mentor" value="{{ $participant->mentor_fullname }}">
                            <small class="form-text text-muted font-italic">(Доступен для редактирования)</small>
                        </div>
                    @endif
